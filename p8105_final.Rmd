...
```{r}
usa_sentiments = us_tweets %>%
  separate(place_as_appears_on_bio, into = c("state", "delete"), ",") %>%
  filter(delete == "USA") 
  
select("place_as_appears_on_bio", "negative", "positive")
  
us_tweets %>%
  filter(country == "US") %>%
  select("place_as_appears_on_bio", "negative", "positive") %>%
  separate(place_as_appears_on_bio, into = c("city", "state"), ",") %>%
  na.omit(state) %>%
  group_by(state) %>%
...
state_tweets = us_tweets %>%
  select("X1", "longitude", "latitude") 

###KATHRYN: try detaching package if it is interfering with the packages you need
...
latlong2state <- function(state_tweets) {
    ...
    states_tweets_SP <- SpatialPoints(states_tweets, 
                    proj4string=CRS("+proj=longlat +datum=WGS84"))
